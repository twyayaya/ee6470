...
void Testbench::do_sobel() {
      ...
      for (v = -1; v != filterHeight-1; ++v) {   //-1, 0, 1
        for (u = -1; u !=filterWidth-1; ++u) { //-1, 0, 1
          if (x + u >= 0 && x + u < width && y + v >= 0 && y + v < height) {
            R += *(source_bitmap +
                  bytes_per_pixel * (width * (y + v) + (x + u)) + 2)*filter[u+1][v+1];
            G += *(source_bitmap +
                  bytes_per_pixel * (width * (y + v) + (x + u)) + 1)*filter[u+1][v+1];
            B += *(source_bitmap +
                  bytes_per_pixel * (width * (y + v) + (x + u)) + 0)*filter[u+1][v+1];
          } //else {
            //R = 0;
            //G = 0;
            //B = 0;
  ...
  cout<<"pixels send  = "<<pixel_number<<endl;
  sc_stop();
}
