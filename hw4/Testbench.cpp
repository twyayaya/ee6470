...
int Testbench::read_bmp(const char *fname_s)
{
...
int Testbench::write_bmp(const char *fname_t)
{
    ...
    for (x = 0; x != width; ++x)
    {
      ...
      { //-1, 0, 1
        ...
        { //-1, 0, 1
          ...
          mask[2] = 0xff;
          mask[3] = 0;
          initiator.write_to_socket(SOBEL_MM_BASE + SOBEL_FILTER_R_ADDR, mask, buf, 4);
        }
      }
      unsigned char buf[4];
      unsigned char mask[4];
      initiator.read_from_socket(SOBEL_MM_BASE + SOBEL_FILTER_RESULT_ADDR, mask, buf, 4);
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = buf[0];
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = buf[1];
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = buf[2];
    }
  }
  sc_stop();
}
...
void Testbench::out()
{
  ...
  for (y = 0; y != height; ++y)
  {
    for (x = 0; x != width; ++x)
    {
      initiator.read_from_socket(SOBEL_FILTER_RESULT_ADDR, mask, buf, 4);
