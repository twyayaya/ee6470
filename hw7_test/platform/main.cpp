...
public:
	...
	addr_t sobelFilter_size = 0x00100000;
	addr_t sobelFilter_end_addr = sobelFilter_start_addr + sobelFilter_size - 1;
	addr_t sobelFilter_2_start_addr = 0x74000000; // change RS to 2
	addr_t sobelFilter_2_size = 0x00100000;
	addr_t sobelFilter_2_end_addr = sobelFilter_2_start_addr + sobelFilter_2_size - 1;
	...
	BasicOptions(void)
	{
		...
		add_options()
			...
			("quiet", po::bool_switch(&quiet), "do not output register values on exit")	//new
	...
	void parse(int argc, char **argv) override
	{
		...
		assert((mem_end_addr < clint_start_addr || mem_start_addr > sobelFilter_2_end_addr) &&
			   "RAM too big, would overlap memory"); //new
...
int sc_main(int argc, char **argv)
{
	...
	ISS core1(1, opt.use_E_base_isa); //new
	...
	CombinedMemoryInterface core1_mem_if("MemoryInterface1", core1); //new
	...
	SobelFilter sobel_filter_2("sobel_filter_2");
	//new

	MemoryDMI dmi = MemoryDMI::create_start_size_mapping(mem.data, opt.mem_start_addr, mem.size);
	InstrMemoryProxy core0_instr_mem(dmi, core0);
	InstrMemoryProxy core1_instr_mem(dmi, core1); //new

	std::shared_ptr<BusLock> bus_lock = std::make_shared<BusLock>();
	core0_mem_if.bus_lock = bus_lock;
	core1_mem_if.bus_lock = bus_lock; //new

	instr_memory_if *core0_instr_mem_if = &core0_mem_if;
	instr_memory_if *core1_instr_mem_if = &core1_mem_if; //iss_mem_if	//new

	data_memory_if *core0_data_mem_if = &core0_mem_if; //iss_mem_if
	data_memory_if *core1_data_mem_if = &core1_mem_if; //new

	if (opt.use_instr_dmi)
	{
		core0_instr_mem_if = &core0_instr_mem;
		core1_instr_mem_if = &core1_instr_mem; //new
	}
	if (opt.use_data_dmi)
	{
		core0_mem_if.dmi_ranges.emplace_back(dmi);
		core1_mem_if.dmi_ranges.emplace_back(dmi); //new
	}
	...
	bus.ports[13] = new PortMapping(opt.sobelFilter_2_start_addr, opt.sobelFilter_2_end_addr); //new

	// connect TLM sockets
	core0_mem_if.isock.bind(bus.tsocks[0]);
	core1_mem_if.isock.bind(bus.tsocks[1]);
	core0_dbg_if.isock.bind(bus.tsocks[2]);
	core1_dbg_if.isock.bind(bus.tsocks[3]); //new
	...
	bus.isocks[13].bind(sobel_filter_2.tsock); //new

	// connect interrupt signals/communication
	plic.target_harts[0] = &core0;
	plic.target_harts[1] = &core1; //new
	clint.target_harts[0] = &core0;
	clint.target_harts[1] = &core1; //new
	...
	threads.push_back(&core1); //new

	core0.trace = opt.trace_mode; // switch for printing instructions
	core1.trace = opt.trace_mode; //new
	if (opt.use_debug_runner)
	{
		auto server0 = new GDBServer("GDBServer0", threads, &core0_dbg_if, opt.debug_port);
		auto server1 = new GDBServer("GDBServer1", threads, &core1_dbg_if, opt.debug_port); //new
		new GDBServerRunner("GDBRunner0", server0, &core0);
		new GDBServerRunner("GDBRunner1", server1, &core1); //new
	}
	else
	{
		new DirectCoreRunner(core0);
		new DirectCoreRunner(core1); //new
	}
